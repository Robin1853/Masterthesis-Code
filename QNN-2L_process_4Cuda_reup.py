...
def qnode2(inputs, weights):
    ...
    qml.RX(inputs[4], wires=0)
    qml.RX(inputs[5], wires=1)
    qml.RX(inputs[6], wires=2)
    qml.RX(inputs[7], wires=3)
    ...
    qml.RX(inputs[8], wires=0)
    qml.RX(inputs[9], wires=1)
    qml.RX(inputs[10], wires=2)
    qml.RX(inputs[11], wires=3)

    qml.Rot(weights[2][0][0], weights[2][0][1], weights[2][0][2], wires=0)
    qml.Rot(weights[2][1][0], weights[2][1][1], weights[2][1][2], wires=1)
    qml.Rot(weights[2][2][0], weights[2][2][1], weights[2][2][2], wires=2)
    qml.Rot(weights[2][3][0], weights[2][3][1], weights[2][3][2], wires=3)

    qml.RX(inputs[12], wires=0)
    qml.RX(inputs[13], wires=1)
    qml.RX(inputs[14], wires=2)
    qml.RX(inputs[15], wires=3)

    qml.Rot(weights[3][0][0], weights[3][0][1], weights[3][0][2], wires=0)
    qml.Rot(weights[3][1][0], weights[3][1][1], weights[3][1][2], wires=1)
    qml.Rot(weights[3][2][0], weights[3][2][1], weights[3][2][2], wires=2)
    qml.Rot(weights[3][3][0], weights[3][3][1], weights[3][3][2], wires=3)
...
def filtering(x):
    ...
    pic_bat = np.swapaxes(pic_bat, 2, 3)
    pic_bat = pic_bat.reshape(bat, 7, 7, 4, 4)
    pic_bat = np.swapaxes(pic_bat, 3, 4)
...
qml.draw_mpl(qnode2, decimals=2)(inputs=['inp1', 'inp2', 'inp3', 'inp4','inp5', 'inp6', 'inp7', 'inp8','inp9', 'inp10', 'inp11', 'inp12','inp13', 'inp14', 'inp15', 'inp16'], weights=[[['w111', 'w112', 'w113'],['w121', 'w122', 'w123'],['w131', 'w132', 'w133'],['w141', 'w142', 'w143']],
                                                                                                                                                                                       [['w211', 'w212', 'w213'],['w221', 'w222', 'w223'],['w231', 'w232', 'w233'],['w241', 'w242', 'w243']],
                                                                                                                                                                                       [['w311', 'w312', 'w313'],['w321', 'w322', 'w323'],['w331', 'w332', 'w333'],['w341', 'w342', 'w343']],
                                                                                                                                                                                       [['w411', 'w412', 'w413'],['w421', 'w422', 'w423'],['w431', 'w432', 'w433'],['w441', 'w442', 'w443']]])
...
for batch_size in batches:
    ...
    # if __name__ == '__main__':
        ...
        #         print(f"Batch: {batch_size} Seed: {seed}/{n_seeds} & Epoch {epoch +1}/{n_epochs}")
        #         #   ###Training###
        #         model.train()
        ...
        #
        #         trainingEpoch_loss.to('cpu')
        #         train_accuracy.to('cpu')
        #
        #         ###Testing
        #         model.eval()
        #
